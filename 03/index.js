...
app.get("/states/highest-gdp", (req, res) => {
  ...
  const highest_gdp = states.reduce((max, state) => {
...
//update only population value only
...
app.delete("/states/name/:stateName", (req, res) => {
  ...
  const index = states.findIndex((s) => s.name.toLowerCase().includes(stateName.toLowerCase()));
